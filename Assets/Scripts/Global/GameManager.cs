    ...
    IEnumerator StartNextWave()
    {
        while(true)
        {
            if(currentSpawnCount == 0)
            {
                ...
                {
                    int posIdx = Random.Range(0, waveSpawnPosCount);
                    for(int j = 0; j < waveSpawnCount; j++)
                    {
                        int prefabIdx = Random.Range(0,enemyPrefabs.Count);
                        GameObject enemy = Instantiate(enemyPrefabs[prefabIdx], spawnPostions[posIdx].position, Quaternion.identity);
    ...
    void RandomUpgrade()
    {
        switch(Random.Range(0,6))
        {
            ...
            case 4:
                ...
                break;
            case 5:
                RangedAttackData randgedAttackData = rangeStats.attackSO as RangedAttackData;
                randgedAttackData.numberofProjectilesPerShot += 1;
                break;
            default:
                break;
        }
